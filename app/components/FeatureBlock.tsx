...
export default async function FeatureBlock({ block, first, last }: Props) {
								...
								<div key={i} className="relative">
									...
									<div className={`relative flex h-full flex-col overflow-hidden rounded-[calc(var(--radius-lg)+1px)] ${cls.inner}`}>
										...
										<div className="p-6 pt-4">
											<p className="flex items-center gap-x-2 text-lg font-semibold tracking-tight text-white">
											{item.icon && <item.icon aria-hidden="true" className="size-5 flex-none text-indigo-400" />}
											{item.title}
										</p>
	...
	if (block.type === "code") {
		const highlightedHtml = block.code ? await highlight(block.code, "bash") : ""

		return (
			<div className={`overflow-hidden${first && last ? "" : first ? " pb-12" : last ? " pt-12" : " py-12"}`}>
				<div className="mx-auto max-w-7xl px-6 lg:px-8">
					<div className={`mx-auto${block.image ? "" : " max-w-3xl"} sm:text-center`}>
					...
					<div className="mx-auto max-w-7xl px-6 lg:px-8">
						...
						{block.code && (
							<CodeBlock
								...
								codeHeight={block.codeHeight}
								className={block.image ? "mt-6" : ""}
							/>
						)}
					</div>
				</div>
